import type { EntityState, Quorum, Address, Input, Command, EntityTx } from './types'
...
export const encodeEntityState = (state: EntityState): RlpInput => {
  ...
  const quorum = [
    state.quorum.threshold.toString(),
    quorumMembers
  ]
  
  // Convert signerRecords to array format
  const signerRecords = Object.entries(state.signerRecords).map(([address, record]) => [
    address,
