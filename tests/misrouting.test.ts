...
import type { ServerState, ServerInput, EntityTx, Address } from '../src/core/types'
...
describe('Mis-routing and Proposer Timeout Tests', () => {
  ...
  describe('Proposer Rotation', () => {
    it('should allow any signer to propose identical tx list after timeout', async () => {
      ...
      const { next: stateWithProposal } = applyServerFrame(stateWithTxs, proposal1, now)
      ...
      const { next: stateWithNewProposal } = applyServerFrame(stateWithProposal, proposal2, now)
      
      // The new proposal would override the old one
      const finalReplica = stateWithNewProposal.get('1:chat')!
      expect(finalReplica.state.proposal?.header.proposer).toBe(signer2.address)
    })
  })
