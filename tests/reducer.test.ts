...
import type { ServerState, ServerInput, EntityTx, Command, Address } from '../src/core/types'
import { createChatTx } from './helpers/tx'
import { signerKeyPair } from './helpers/crypto'

describe('Reducer - v1.4.1-RC2 Compliance', () => {
  const signer1 = { address: '0x0000000000000000000000000000000000000001' as Address }
  const signer2 = { address: '0x0000000000000000000000000000000000000002' as Address }
  const signer3 = { address: '0x0000000000000000000000000000000000000003' as Address }
  ...
  describe('Nonce Monotonicity', () => {
    it('should reject transaction with incorrect nonce', async () => {
      ...
      const entityState = {
        height: 0n,
        quorum: { threshold: 1n, members: [{ address: signer1.address, shares: 1n }] },
  ...
  describe('SignerIdx Ordering (A1)', () => {
    it('should enforce lexicographic signerIdx ordering', async () => {
      ...
      expect(() => applyServerFrame(state, input, now)).toThrow('signerIdx mismatch')
    })
    ...
    it('should accept valid signerIdx ordering', async () => {
      ...
      const input: ServerInput = {
        ...
        timestamp: now(),
        inputs: [
          [0, 'test', { type: 'importEntity', snapshot: {
            height: 0n,
          ...
          [1, 'test2', { type: 'importEntity', snapshot: {
            height: 0n,
            quorum: { threshold: 1n, members: [] },
            signerRecords: {},
            domainState: {},
  ...
  describe('Frame Hash (R-1)', () => {
    it('should compute frame hash as keccak256(rlp(header ‖ txs))', async () => {
      ...
      const { next: proposed } = applyServerFrame(stateWithTx, proposeInput, now)
